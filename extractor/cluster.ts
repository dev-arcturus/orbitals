...
export async function runClusterPass(
  ...
  try {
    return await callLLM(graph, fileSummaries);
  } catch (err) {
...
async function callLLM(
  ...
  const prompt = buildPrompt(graph, fileSummaries);
...
function buildPrompt(
  ...
  return [
    ...
    "Every node must belong to exactly one cluster. Use the file paths and short file summaries to inform grouping.",
